...
class V2406(snmp: SnmpContext) : BaseHandler(snmp) {
    companion object {
        val vendor = "gateray"
        val model = "gr-ep-olt1-8"
        val softwareVersion = "2.4.06_000(Nov 11 2020)"
    }

    override fun discover() {
        snmp.subtree(".1.3.6.1.4.1.34592.1.3.4.1.1.7.1", { e ->
            e?.variableBindings?.associate {
                val (root, port, position) = it.oid.value.takeLast(3).toTypedArray()
